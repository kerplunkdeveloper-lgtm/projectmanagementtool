...
const Navbar = ({ setSidebarOpen }) => {
  ...
  const { data: projects = [] } = useGetProjectsQuery(undefined, { skip: !user });
  ...
  const getSearchItems = () => {
    ...
    // 1. Pages / Navigation
    const pages = [
      { id: "nav-dashboard", category: "Navigation", title: "Go to Dashboard", path: `/${user?.role}` },
      { id: "nav-projects", category: "Navigation", title: "Go to Projects", path: `/${user?.role}/projects` },
      { id: "nav-tasks", category: "Navigation", title: "Go to Tasks", path: `/${user?.role}/tasks` },
      { id: "nav-clients", category: "Navigation", title: "Go to Clients", path: `/${user?.role}/clients` },
      { id: "nav-portfolio", category: "Navigation", title: "Go to Portfolio Groups", path: `/${user?.role}/portfolio` },
      { id: "nav-calendar", category: "Navigation", title: "Go to Calendar", path: `/${user?.role}/calendar` },
      { id: "nav-chat", category: "Navigation", title: "Go to Chat / Messages", path: `/${user?.role}/chat` },
      { id: "nav-workload", category: "Navigation", title: "Go to Workload", path: `/${user?.role}/workload` },
      { id: "nav-settings", category: "Navigation", title: "Go to Settings", path: `/${user?.role}/settings` },
      { id: "nav-profile", category: "Navigation", title: "Go to Profile", path: `/${user?.role}/profile` },
    ];
      ...
      <AnimatePresence>
        {showSearchModal && (
          <motion.div 
            ...
            className="fixed inset-0 bg-slate-950/70 backdrop-blur-xl z-[9999] flex items-start justify-center pt-10 sm:pt-20 px-3 sm:px-4"
          ...
          >
            <motion.div
              ...
              className="w-full max-h-[85vh] sm:h-auto sm:max-w-2xl bg-slate-50/95 dark:bg-[#020710]/95 backdrop-blur-md border border-slate-200/60 dark:border-slate-800/60 rounded-2xl shadow-2xl shadow-indigo-500/10 overflow-hidden flex flex-col"
            ...
            >
              ...
              <div className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-200 dark:border-slate-800/60">
                ...
                <button
                  onClick={() => setShowSearchModal(false)}
                  className="p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800/60 text-slate-400 hover:text-slate-650 dark:hover:text-slate-200 transition-all cursor-pointer flex items-center justify-center shrink-0"
                  title="Close Search"
                >
                  <FiX size={16} />
                </button>
              ...
              <div className="flex-1 sm:max-h-[380px] overflow-y-auto p-2 scrollbar-thin">
                {allFilteredItems.length === 0 ? (
                  ...
                  <div>
                    {/* We render grouped categories */}
                    {(() => {
                      let globalItemIndex = 0;
                      return categories.map((cat) => {
                        const catItems = allFilteredItems.filter(item => item.category === cat);
                        ...
                        return (
                          <div key={cat} className="mb-3">
                            ...
                            <div className="space-y-0.5">
                              {catItems.map((item) => {
                                const isHighlighted = globalItemIndex === selectedIndex;
                                ...
                                return (
                                  <div
                                    ...
                                    className={`
                                      ...
                                      ${isHighlighted 
                                        ? "bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-305" 
                                        : "hover:bg-slate-200/50 dark:hover:bg-slate-800/40 text-slate-700 dark:text-slate-300"
                                      }
                                    `}
                                  >
                                    <div className="flex items-center gap-3 min-w-0">
                                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center border border-slate-200 dark:border-slate-800 shrink-0 ${isHighlighted ? "bg-indigo-500/20" : "bg-slate-100 dark:bg-slate-850"}`}>
                                        {item.type === "navigation" && <FiCompass size={13} />}
                                        {item.type === "project" && <FiBriefcase size={13} />}
                                        {item.type === "task" && <FiCheckSquare size={13} />}
                                        {item.type === "client" && <FiUser size={13} />}
                                      </div>
                                    ...
                                    {isHighlighted && (
                                      <span className="text-[9px] font-black uppercase tracking-wider text-indigo-650 dark:text-indigo-400 shrink-0 mr-1 animate-pulse">
                                        ⏎ Go
                                      </span>
                                    )}
                                  </div>
              ...
              <div className="hidden sm:flex px-4 py-2.5 bg-slate-105/50 dark:bg-[#020710]/50 border-t border-slate-200 dark:border-slate-800/60 items-center justify-between text-[10px] text-slate-400 dark:text-slate-500 font-semibold select-none">
