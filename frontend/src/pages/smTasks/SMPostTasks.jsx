...
import {
  ...
  deleteSMTask,
  clearAllSMTasks,
} from "../../features/smTasks/smTaskApi";
...
const statusStyles = {
  ...
  Scheduled:
    "bg-cyan-50 text-cyan-600 border-cyan-200 dark:bg-cyan-950/40 dark:text-cyan-400",
  Completed:
...
const SMPostTasks = () => {
  ...
  const [newTaskPlatforms, setNewTaskPlatforms] = useState([
    "Instagram",
    "Facebook",
  ]);
  ...
  const fetchData = async () => {
    try {
      ...
      const uData = Array.isArray(usersRes?.data)
        ? usersRes.data
  ...
  const filteredTasks = useMemo(() => {
    ...
    return safeTasks.filter((task) => {
      ...
      if (activeFilterTab === "dueToday")
        return isToday(task.dueDate) && task.status !== "Completed";
      if (activeFilterTab === "overdue")
        return (
          isOverdue(task.dueDate, task.status) || task.status === "Blocked"
        );
  ...
  const handleDeleteTask = async (taskId) => {
    if (!window.confirm("Are you sure you want to delete this SM Task?"))
      return;
    try {
  ...
  const handleClearAllTasks = async () => {
    if (
      !window.confirm(
        "Are you sure you want to clear ALL tasks from the table and database?",
      )
    )
      return;
    try {
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2">
          ...
          <div className="bg-white dark:bg-[#18233c] p-3 rounded-xl border border-slate-200/80 dark:border-white/10 shadow-xs flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-base font-bold text-slate-900 dark:text-white mt-1 leading-none">
                {stats.total}
              </div>
            </div>
          </div>
            ...
            {[
              {
                ...
                count: tasks.length,
                badgeStyle:
                  "active:bg-blue-600 active:text-white bg-blue-600 text-white",
              },
              {
                id: "today",
                label: "Today's Tasks",
                ...
                >
                  ...
                  <span
                    className={`px-1.5 py-0.2 rounded-full text-[9px] font-bold ${
                      isActive ? "bg-blue-600 text-white" : tab.badgeStyle
                    }`}
                  >
                    {tab.count}
                  </span>
        ...
        <div className="bg-white dark:bg-[#18233c] rounded-2xl border border-slate-200/80 dark:border-white/10 shadow-sm flex flex-col justify-between min-h-[520px] lg:min-h-[calc(100vh-250px)] overflow-hidden">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {loading ? (
                    ...
                    >
                      <div className="max-w-xs mx-auto space-y-2">
                        ...
                        <p className="text-xs text-slate-400">
                          Create your first social media task using the Quick
                          Add sidebar or templates.
                        </p>
                      </div>
                    </td>
                  </tr>
                  ...
                  paginatedTasks.map((t) => {
                    const clientName =
                      t.client?.companyName || "Unknown Client";
                      ...
                      >
                        ...
                        <td className="py-2 px-2.5">
                          <div className="flex items-center gap-1.5 text-sm">
                            ...
                            {(t.platform?.includes("Google My Business") ||
                              t.platform?.includes("Google Business")) && (
                              <FaGoogle
                                className="text-emerald-600 dark:text-emerald-400"
                                title="Google My Business"
                              />
                            )}
                            {(t.platform?.includes("X") ||
                              t.platform?.includes("Twitter")) && (
                              <FaTwitter
                        ...
                        <td className="py-2 px-2.5 whitespace-nowrap">
                          {(() => {
                            ...
                            const userName =
                              userObj.name || currentUser?.name || "Dharani H";
                            const department =
                              userObj.department ||
          ...
          <div className="p-4 border-t border-slate-200/80 dark:border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
            <div>
              ...
              to {Math.min(currentPage * tasksPerPage, filteredTasks.length)} of{" "}
        ...
        <div className="bg-blue-50/50 dark:bg-blue-950/20 p-3 rounded-xl border border-blue-100 dark:border-blue-900/30 flex flex-col md:flex-row items-center justify-between gap-3 text-xs">
          ...
          <div className="text-blue-700 dark:text-blue-300 font-medium flex items-center gap-1.5">
            ...
            <span>
              Quick Tip: Use quick templates or set recurring tasks to save
              time.
            </span>
          </div>
        </div>
      ...
      {isEditModalOpen && editingTask && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#18233c] w-full max-w-lg rounded-2xl border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/10 pb-3">
              <h3 className="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-2">
                <FiEdit className="text-blue-600" /> Edit Social Media Task (
                {editingTask.taskId})
              </h3>
              <button
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  {[
                    ...
                    const isSelected = (editingTask.platform || []).includes(
                      plat,
                    );
                    return (
                      <button
