...
const TYPE_CONFIG = {
  Onboarding:      { color: "bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20",  dot: "bg-emerald-500 dark:bg-emerald-400" },
  "Service Process":{ color: "bg-violet-100 dark:bg-violet-500/10 text-violet-700 dark:text-violet-400 border-violet-200 dark:border-violet-500/20",    dot: "bg-violet-500 dark:bg-violet-400" },
  Checklist:       { color: "bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/20",        dot: "bg-amber-500 dark:bg-amber-400" },
  Campaign:        { color: "bg-blue-100 dark:bg-[#e5ff00]/10 text-blue-700 dark:text-[#e5ff00] border-blue-200 dark:border-[#e5ff00]/20",           dot: "bg-blue-500 dark:bg-[#e5ff00]" },
};

const SERVICE_COLORS = {
  SMM:   "bg-blue-50 dark:bg-[#e5ff00]/10 text-blue-600 dark:text-[#e5ff00] border-blue-100 dark:border-[#e5ff00]/20",
  ...
  Video: "bg-pink-50 dark:bg-pink-500/10 text-pink-600 dark:text-pink-400 border-pink-100 dark:border-pink-500/20",
};
...
const Templatelib = () => {
    ...
    <div className="min-h-screen">
      <div className="py-4 sm:py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <div className="w-7 h-7 rounded-lg bg-blue-50 dark:bg-[#e5ff00]/10 border border-blue-100 dark:border-[#e5ff00]/20 flex items-center justify-center">
                <FiLayers size={14} className="text-blue-600 dark:text-[#e5ff00]" />
              </div>
          ...
          <button
            onClick={openCreate}
            className="self-start sm:self-auto flex items-center gap-1.5 px-4 py-2 text-[11px] rounded-xl bg-blue-600 dark:bg-[#e5ff00] text-white dark:text-black shadow-lg shadow-blue-500/20 dark:shadow-[#e5ff00]/20 hover:-translate-y-0.5 active:scale-95 transition-all duration-200 font-black uppercase tracking-wider shrink-0"
          >
            <IoAdd size={16} />
            New Template
          </button>
  ...
  <button
    ...
    className={`flex-shrink-0 px-3 py-1.5 rounded-lg whitespace-nowrap text-xs font-semibold transition-all duration-200 ${
      activeTab === tab.id
        ? "bg-blue-600 dark:bg-[#e5ff00] text-white dark:text-black shadow-md dark:shadow-[#e5ff00]/20"
        : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-[#e5ff00] hover:bg-gray-50 dark:hover:bg-white/10"
    }`}
  >
    {tab.label}
  </button>
          ...
          {/* SEARCH */}
          <div className="flex items-center gap-2 bg-white dark:bg-[#111111] border border-gray-200 dark:border-white/5 rounded-xl px-3 py-2 w-full sm:w-56 focus-within:border-blue-400 dark:focus-within:border-[#e5ff00] transition-colors">
        ...
        {filtered.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {filtered.map((template) => {
              ...
              return (
                  <div
                  key={template._id}
                  className="group bg-white dark:bg-[#111111] border border-slate-200 dark:border-white/5 rounded-2xl p-4 shadow-lg hover:shadow-md hover:border-blue-400 dark:hover:border-[#e5ff00] hover:-translate-y-0.5 transition-all duration-300 flex flex-col"
                >
                  {/* CARD TOP */}
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div className="flex-1 min-w-0">
                      <h2 className="text-sm font-bold text-slate-800 dark:text-white group-hover:text-blue-600 dark:group-hover:text-[#e5ff00] transition-colors truncate">
                        {template.title}
                      </h2>
                      <span
                  ...
                  <p className="text-xs text-gray-500 dark:text-slate-400 leading-relaxed line-clamp-2 flex-1 mb-3">
                    {template.description || (
                      <span className="italic text-gray-300 dark:text-slate-600">No description</span>
                    )}
                  </p>
                      ...
                      {template.services.map((s) => (
                        <span
                          ...
                          className={`px-2 py-0.5 rounded-lg text-[10px] font-bold border ${
                            SERVICE_COLORS[s] || "bg-gray-50 dark:bg-white/5 text-gray-600 dark:text-slate-400 border-gray-100 dark:border-white/10"
                          }`}
                        >
                          {s}
                        </span>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white dark:bg-[#111111] border border-slate-200 dark:border-white/5 rounded-3xl shadow-sm">
            <div className="w-14 h-14 bg-gray-100 dark:bg-white/5 rounded-2xl flex items-center justify-center mb-3">
              <FiLayers size={24} className="text-gray-300 dark:text-slate-600" />
            </div>
            <h2 className="text-base font-bold text-slate-600 dark:text-white">No Templates Found</h2>
            ...
            {!search && (
              <button
                onClick={openCreate}
                className="mt-4 flex items-center gap-1.5 px-4 py-2.5 text-[10px] rounded-xl bg-blue-600 dark:bg-[#e5ff00] text-white dark:text-black shadow-lg shadow-blue-500/20 dark:shadow-[#e5ff00]/20 hover:-translate-y-0.5 active:scale-95 transition-all duration-200 font-black uppercase tracking-wider"
              >
            ...
            <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">
                  Template Name <span className="text-rose-400 dark:text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full h-9 bg-gray-50 dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/10 rounded-xl px-3 outline-none focus:border-blue-400 dark:focus:border-[#e5ff00] focus:ring-2 focus:ring-blue-100 dark:focus:ring-[#e5ff00]/20 transition-all text-sm text-slate-700 dark:text-white cursor-pointer"
                  >
                    <option className="dark:bg-[#111111]">Onboarding</option>
                    <option className="dark:bg-[#111111]">Service Process</option>
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold border transition-all ${
                          formData.services.includes(s)
                            ? "bg-blue-600 dark:bg-[#e5ff00] text-white dark:text-black border-blue-600 dark:border-[#e5ff00]"
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-gray-100 dark:border-white/5">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 text-[10px] rounded-xl bg-blue-600 dark:bg-[#e5ff00] hover:bg-blue-700 dark:hover:bg-[#ccff00] text-white dark:text-black font-black uppercase tracking-wider shadow-lg shadow-blue-500/20 dark:shadow-[#e5ff00]/20 transition-all hover:-translate-y-0.5 active:scale-95"
                >
