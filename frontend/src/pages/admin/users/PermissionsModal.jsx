...
const PERMISSION_MODULES = [
  { id: "manage_users", label: "Manage Users", desc: "Create, edit, and delete user accounts" },
  { id: "manage_roles", label: "Roles & Permissions", desc: "Assign module access and modify permissions" },
  { id: "manage_projects", label: "Manage Projects", desc: "Create and edit business projects" },
  { id: "manage_tasks", label: "Manage Tasks", desc: "Create, assign, and update tasks" },
  { id: "view_reports", label: "View Reports", desc: "Access EOD reports and analytics" },
  { id: "manage_clients", label: "Manage Clients", desc: "Add and edit client details" },
  { id: "manage_settings", label: "System Settings", desc: "Configure global application settings" },
];
...
const PermissionsModal = ({ open, setOpen, user, handleUpdateUser }) => {
  ...
  const handleToggle = (moduleId, action) => {
    ...
    setPermissions(prev => {
      const currentModulePerms = prev[moduleId] || { read: false, write: false, update: false, delete: false };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-500 dark:text-slate-400 font-medium mt-0.5">
                Manage access for <span className="text-slate-700 dark:text-slate-300 font-bold">{user.name}</span>
              </p>
            </div>
          </div>
          <button
                    ...
                    return (
                      <tr key={module.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                        ...
                        {PERMISSION_ACTIONS.map(action => {
                          ...
                          const isGranted = isAdmin || isLegacyTrue || modulePerms[action.id];
                          
                          return (
                            <td key={action.id} className="px-4 py-3 text-center align-middle">
                              <label className={`relative flex items-center justify-center cursor-pointer ${isAdmin ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                <input 
                                  type="checkbox" 
                                  className="peer sr-only"
                                  checked={!!isGranted}
                                  onChange={() => handleToggle(module.id, action.id)}
                                  disabled={isAdmin}
                                />
                                <div className={`
