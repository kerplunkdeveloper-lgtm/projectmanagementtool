...
const Portfolio = () => {
  ...
  useEffect(() => {
    ...
    const savedPortfolios = localStorage.getItem("workspace_portfolios");
    if (savedPortfolios) {
      ...
      localStorage.setItem("workspace_portfolios", JSON.stringify(defaultPortfolios));
    }
  }, [dispatch]);

  // Persist portfolios helper
  const savePortfolios = (updated) => {
    setPortfolios(updated);
    localStorage.setItem("workspace_portfolios", JSON.stringify(updated));
  };
  ...
  const handleSavePortfolio = (e) => {
    ...
    if (isEditMode) {
      ...
      const updated = [...portfolios, newFolder];
      savePortfolios(updated);
    }
  ...
  const handleBatchAddProjects = () => {
    if (selectedAddProjects.length === 0) return;
    const updated = portfolios.map((p) => {
      if (p.id === selectedPortfolioId) {
        const existing = p.projectIds || [];
        const merged = Array.from(new Set([...existing, ...selectedAddProjects]));
        return { ...p, projectIds: merged };
      }
      return p;
    });
    savePortfolios(updated);
  ...
  const handleRemoveProject = (projectId) => {
    const updated = portfolios.map((p) => {
      if (p.id === selectedPortfolioId) {
        const projectIds = p.projectIds || [];
        return { ...p, projectIds: projectIds.filter((id) => id !== projectId) };
      }
      return p;
    });
    savePortfolios(updated);
  ...
  if (selectedPortfolioId && !activePortfolio) {
    return (
      <div className="p-8 text-center max-w-md mx-auto mt-12 bg-white dark:bg-slate-800 rounded-2xl border border-slate-150 dark:border-slate-700/60 shadow-sm">
        <p className="text-slate-500 dark:text-slate-400 font-medium">Resolving portfolio group...</p>
        <button
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6 transition-all duration-300">
      <AnimatePresence mode="wait">
        {!selectedPortfolioId ? (
          ...
          >
            <div>
              <h1 className="text-xl font-extrabold text-slate-800 dark:text-white">Portfolios Dashboard</h1>
              <p className="text-slate-500 dark:text-slate-400 text-[10px] mt-1">
                Group projects into custom folder portfolios for high-level management
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              ...
              >
                <div className="w-12 h-12 rounded-xl border border-slate-250 dark:border-slate-750 flex items-center justify-center text-slate-400 group-hover:text-blue-500 group-hover:border-blue-300 dark:group-hover:border-blue-800 transition-all shadow-inner">
                  <FiPlus size={22} className="group-hover:scale-110 transition-transform" />
                </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative w-36 h-28 flex items-center justify-center shrink-0">
                      <svg viewBox="0 0 240 180" className="w-full h-full drop-shadow-md transition-transform duration-300 group-hover:scale-105" style={{ fill: portfolio.color || "#ff80bf" }}>
                      ...
                      >
                        <FiStar size={15} className={portfolio.isFavorite ? "fill-white text-white" : ""} />
                      </button>

                      {/* Actions Menu Trigger (Top-Right inside folder) */}
                      <div className="absolute top-7 right-4" onClick={(e) => e.stopPropagation()}>
                        <button
                          onClick={() => setMenuOpenId(menuOpenId === portfolio.id ? null : portfolio.id)}
                        ...
                        {menuOpenId === portfolio.id && (
                          <div className="absolute right-0 mt-1.5 w-28 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-150 dark:border-slate-700 py-1 z-30">
                            ...
                            >
                              <FiEdit3 size={12} className="text-slate-400" /> Edit Details
                            </button>
                            <button
                              onClick={(e) => {
                                handleDeletePortfolio(e, portfolio.id);
                                setMenuOpenId(null);
                              }}
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-slate-550 font-bold uppercase mt-1">
                      {portfolio.projectIds?.length || 0} Project{(portfolio.projectIds?.length !== 1) ? "s" : ""}
                    </p>
                  </div>
                );
              })}
          ...
          >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-2">
              <div className="flex items-center gap-3">
                <div 
                  ...
                  style={{ backgroundColor: activePortfolio.color || "#ff80bf" }}
                >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase mt-0.5 tracking-wider">
                    Portfolio Group Workspace • {activePortfolio.projectIds?.length || 0} Project{(activePortfolio.projectIds?.length !== 1) ? "s" : ""}
                  </p>
                </div>
              </div>
            ...
            <div className="min-h-[350px] space-y-4">
                  {/* Toolbar */}
                  <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-50/50 dark:bg-slate-900/40 p-3 rounded-2xl border border-slate-150 dark:border-slate-800/80">
                    <div>
                      <button
                        onClick={() => {
                          setSelectedAddProjects([]);
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-750 hover:border-slate-300 dark:hover:border-slate-600 transition-all shadow-sm active:scale-95 cursor-pointer"
                      >
                        <LuPlus size={14} className="text-slate-500 dark:text-slate-400" />
                        Add work
                      </button>
                    </div>
                  ...
                  <div className="overflow-x-auto bg-white dark:bg-slate-800 rounded-2xl border border-slate-150 dark:border-slate-700/60 shadow-sm">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700/60 text-slate-405 dark:text-slate-400 text-[10px] font-bold uppercase tracking-wider">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-700/60 text-xs">
                        {activePortfolio.projectIds?.length === 0 ? (
                          <tr>
                            <td colSpan={6} className="px-6 py-12 text-center text-slate-400 dark:text-slate-500 italic">
                              No projects added. Choose "+ Add work" or search below to group projects inside this portfolio.
                            </td>
                          </tr>
                        ) : (
                          activePortfolio.projectIds.map((projId) => {
                            const project = projects.find((p) => p._id === projId);
                            if (!project) return null;

                            // Calculate progress percentage
                            const projectTasks = tasks.filter(
                              (t) => t.project?._id === projId || t.project === projId
                            );
                            ...
                            const priorities = projectTasks.map((t) => t.priority).filter(Boolean);
                            ...
                            return (
                              <tr key={projId} className="hover:bg-slate-50/50 dark:hover:bg-slate-750/30 transition-colors">
                                {/* Name */}
                                ...
                                <td className="px-6 py-4">
                                  <span className={`text-[9px] font-black px-2.5 py-1 rounded-lg border uppercase tracking-wider ${getStatusBadge(project.status)}`}>
                                ...
                                <td className="px-6 py-4">
                                  <span className={`px-2 py-0.5 rounded text-[8px] font-extrabold border uppercase tracking-wider ${
                                    highestPriority === "High" ? "bg-rose-50 text-rose-700 border-rose-100 dark:bg-rose-950/20 dark:text-rose-400 dark:border-rose-900/50" :
                                    highestPriority === "Medium" ? "bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900/50" :
            ...
            >
              ...
              {(() => {
                const available = projects
                  .filter((p) => !activePortfolio.projectIds.includes(p._id))
                  .filter((p) => p.name?.toLowerCase().includes(projectSearchQuery.toLowerCase()));
                if (available.length === 0) return null;
                const allSelected = available.every((p) => selectedAddProjects.includes(p._id));
                return (
                  <div className="flex items-center justify-between pb-2.5 mb-2 border-b border-slate-100 dark:border-slate-800/80 px-0.5">
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (allSelected) {
                          setSelectedAddProjects(prev => prev.filter(id => !available.some(a => a._id === id)));
                        } else {
                          setSelectedAddProjects(prev => Array.from(new Set([...prev, ...available.map(a => a._id)])));
                        }
                      }}
              ...
              {/* Project list */}
              <div className="flex-1 overflow-y-auto space-y-1.5 pr-1" style={{ minHeight: '120px', maxHeight: '340px' }}>
                {(() => {
                  const available = projects
                    .filter((p) => !activePortfolio.projectIds.includes(p._id))
                    .filter((p) => p.name?.toLowerCase().includes(projectSearchQuery.toLowerCase()));

                  if (projects.filter(p => !activePortfolio.projectIds.includes(p._id)).length === 0) {
                    return (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center shrink-0 transition-all ${
                          ...
                          {isChecked && <FiCheck size={11} className="text-white dark:text-black font-black" />}
                        </div>
                        ...
                        {/* Status badge */}
                        <span className={`text-[8px] font-black px-2 py-0.5 rounded-lg border uppercase tracking-wider shrink-0 ${getStatusBadge(proj.status)}`}>
                          {proj.status || 'Active'}
                        </span>
                      </div>
                    );
              ...
              <div className="flex items-center justify-between pt-5 mt-4 border-t border-slate-100 dark:border-slate-800/80">
                <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                  {selectedAddProjects.length > 0 ? `${selectedAddProjects.length} selected` : 'None selected'}
                </span>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                  ...
                  >
                    Add {selectedAddProjects.length > 0 ? `(${selectedAddProjects.length})` : ''}
                  </button>
                </div>
              </div>
            </motion.div>
              ...
              <form onSubmit={handleSavePortfolio} className="space-y-6">
                ...
                <div className="space-y-3">
                    ...
                    {[
                      ...
                      >
                        {portfolioColor === col.value && (
                          <FiCheck size={14} className="text-white drop-shadow-sm font-black" />
                        )}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    ...
                    <div>
                      <span className="text-[10px] font-black text-slate-450 dark:text-slate-500 uppercase tracking-wider">Custom color picker</span>
                      <p className="text-[9px] font-mono text-slate-400 dark:text-slate-500 uppercase">{portfolioColor}</p>
                    </div>
                  </div>
                </div>
